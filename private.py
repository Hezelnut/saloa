...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(['전설지도','최상급 오레하 제작','상급 오레하 제작','배틀아이템 공장','컨텐츠 손익','경매 입찰가격'])
...
with tab3:
    ...
    st.write('제작 수수료 기준 : ',int(230),'골드')
    slide_option_2 = (price('상급 오레하 융화 재료')-2,price('상급 오레하 융화 재료')-1,price('상급 오레하 융화 재료'),price('상급 오레하 융화 재료')+1,price('상급 오레하 융화 재료')+2)
    oreha_value_2 = st.selectbox('상급 오레하 판매 가격',slide_option_2,index=2)
    for n in range(0,oreha_value_2):
        if n+1>=oreha_value_2*0.05>n:
            oreha_charge_2 = n+1


    st.subheader('고고학 제작')
    legacy_1_2 = ('오레하 유물 : {}골드, 희귀한 유물 : {}골드, 고대 유물 : {}골드'.format(price('오레하 유물'),price('희귀한 유물'),price('고대 유물')))
    if price('희귀한 유물')*25/4 >= price('오레하 유물'):
        oreha_legacy = price('오레하 유물')
        legacy_2 = '가루 교환 쓰지않음'
    else:
        oreha_legacy = price('희귀한 유물')*25/4
        legacy_2 = '가루 교환 사용'
    recipe_legacy_2 = oreha_legacy*160 + price('희귀한 유물')*290 + price('고대 유물')*94
    profit_legacy_2 = (oreha_value_2-oreha_charge_2)*2000-recipe_legacy_2-23000

    if profit_legacy_2>=0:
        legacy_result_2 = '1칸 당 '+str(int(profit_legacy_2/100))+'골드 이득'
    else:
        legacy_result_2 = '만들어 팔면 손해'
    
    st.metric(label=legacy_1_2,value=legacy_result_2,delta=legacy_2,delta_color='off')
...
with tab5:
    ...
    if '아브렐슈드 하드1관문' not in st.session_state:
        st.session_state['아브렐슈드 하드1관문'] = {'컨텐츠 보상':{'명예의 파편':2000,'정제된 파괴강석':40,'정제된 수호강석':80,'찬란한 명예의 돌파석':0,'혼돈의 돌':0.6,'클리어 골드':2500},'더보기':{'명예의 파편':2400,'정제된 파괴강석':36,'정제된 수호강석':72,'찬란한 명예의 돌파석':2.4,'혼돈의 돌':0.6,'더보기 골드':700}}

    if '아브렐슈드 하드2관문' not in st.session_state:
        st.session_state['아브렐슈드 하드2관문'] = {'컨텐츠 보상':{'명예의 파편':2000,'정제된 파괴강석':40,'정제된 수호강석':80,'찬란한 명예의 돌파석':0,'혼돈의 돌':0.8,'클리어 골드':3000},'더보기':{'명예의 파편':3000,'정제된 파괴강석':54,'정제된 수호강석':108,'찬란한 명예의 돌파석':2.4,'혼돈의 돌':0.8,'더보기 골드':800}}
    
    if '아브렐슈드 하드3관문' not in st.session_state:
        st.session_state['아브렐슈드 하드3관문'] = {'컨텐츠 보상':{'명예의 파편':2000,'정제된 파괴강석':56,'정제된 수호강석':112,'찬란한 명예의 돌파석':0,'혼돈의 돌':0.6,'클리어 골드':900},'더보기':{'명예의 파편':3200,'정제된 파괴강석':72,'정제된 수호강석':144,'찬란한 명예의 돌파석':3.2,'혼돈의 돌':0.6,'더보기 골드':900}}

    if '아브렐슈드 하드4관문' not in st.session_state:
        st.session_state['아브렐슈드 하드4관문'] = {'컨텐츠 보상':{'명예의 파편':2000,'정제된 파괴강석':56,'정제된 수호강석':112,'찬란한 명예의 돌파석':0,'혼돈의 돌':0.8,'클리어 골드':1100},'더보기':{'명예의 파편':4000,'정제된 파괴강석':96,'정제된 수호강석':192,'찬란한 명예의 돌파석':3.2,'혼돈의 돌':0.8,'더보기 골드':1100}}

    if '아브렐슈드 하드5관문' not in st.session_state:
        st.session_state['아브렐슈드 하드5관문'] = {'컨텐츠 보상':{'명예의 파편':3200,'정제된 파괴강석':96,'정제된 수호강석':912,'찬란한 명예의 돌파석':0,'혼돈의 돌':0.6,'클리어 골드':1200},'더보기':{'명예의 파편':4800,'정제된 파괴강석':120,'정제된 수호강석':240,'찬란한 명예의 돌파석':4,'혼돈의 돌':0.6,'더보기 골드':1100}}
    
    if '아브렐슈드 하드6관문' not in st.session_state:
        st.session_state['아브렐슈드 하드6관문'] = {'컨텐츠 보상':{'명예의 파편':3200,'정제된 파괴강석':96,'정제된 수호강석':192,'찬란한 명예의 돌파석':0,'혼돈의 돌':1,'클리어 골드':1800},'더보기':{'명예의 파편':6000,'정제된 파괴강석':180,'정제된 수호강석':360,'찬란한 명예의 돌파석':6,'혼돈의 돌':1,'더보기 골드':1400}}
    ...
    if '일리아칸 노말3관문' not in st.session_state:
        st.session_state['일리아칸 노말3관문'] = {'컨텐츠 보상':{'명예의 파편':4200,'정제된 파괴강석':240,'정제된 수호강석':480,'찬란한 명예의 돌파석':8,'혼돈의 돌':1,'클리어 골드':3000},'더보기':{'명예의 파편':4200,'정제된 파괴강석':240,'정제된 수호강석':480,'찬란한 명예의 돌파석':8,'혼돈의 돌':1,'더보기 골드':1500}}
