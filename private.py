...
tab0, tab1, tab2, tab3, tab4, tab5 = st.tabs(['재료','융화 재료 제작','배틀아이템 제작','4티어 컨텐츠 손익','4티어 악세 파편','경매 입찰가격'])
...
with tab4:
    ...
    shard_legacy = (st.number_input('유물 3연마 가격 = __골드',value=800) + 20*peon) / 24
    shard_ancient = (st.number_input('고대 3연마 가격 = __골드',value=24000) + 35*peon) / 180
...
with tab5:
    ...
    if radio == '4인 컨텐츠':
        ...
        st.write('[손님]보석 거래 가격 : ',int( bus4_1 + (bus4_2-bus4_1)/8),'골드')
        st.write('[손님]독식 입찰 가격 : ',int( (bus4_2-bus4_1)*7/8 ),'골드')
    elif radio == '베히 8인 버스':
        st.write('[유물각인서] 보석 입찰 가격 ',(int(bid*0.95-50)/7.95),'골드')
        behemoth_1 = st.number_input("미참 손님 가격",value=15000)
        behemoth_2 = st.number_input("독식 손님 가격",value=18000)
        st.write('[손님]보석 거래 가격 : ',int( behemoth_1 + (behemoth_2-behemoth_1)/16),'골드')
        st.write('[손님]독식 입찰 가격 : ',int( (behemoth_2-behemoth_1)*15/16 ),'골드')
