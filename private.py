...
with tab4:
    ...
    if '아브렐슈드 하드2관문' not in st.session_state:
        st.session_state['아브렐슈드 하드2관문'] = {'명예의 파편':2000,'정제된 파괴강석':36,'찬란한 명예의 돌파석':1.6,'혼돈의 돌':0.8,'더보기 골드':800}
    
    if '아브렐슈드 하드3관문' not in st.session_state:
        st.session_state['아브렐슈드 하드3관문'] = {'명예의 파편':1600,'정제된 파괴강석':36,'찬란한 명예의 돌파석':1.6,'혼돈의 돌':0.6,'더보기 골드':900}    

    if '아브렐슈드 하드4관문' not in st.session_state:
        st.session_state['아브렐슈드 하드4관문'] = {'명예의 파편':2000,'정제된 파괴강석':48,'찬란한 명예의 돌파석':1.6,'혼돈의 돌':0.8,'더보기 골드':1100}

    if '아브렐슈드 하드5관문' not in st.session_state:
        st.session_state['아브렐슈드 하드5관문'] = {'명예의 파편':2400,'정제된 파괴강석':60,'찬란한 명예의 돌파석':2,'혼돈의 돌':0.6,'더보기 골드':1100}
    
    if '아브렐슈드 하드6관문' not in st.session_state:
        st.session_state['아브렐슈드 하드6관문'] = {'명예의 파편':3000,'정제된 파괴강석':90,'찬란한 명예의 돌파석':3,'혼돈의 돌':1,'더보기 골드':1400}    

    if '천공의 문 넬라시아 하드3' not in st.session_state:
        st.session_state['천공의 문 넬라시아 하드3'] = {'명예의 파편':2400,'정제된 파괴강석':75,'찬란한 명예의 돌파석':3,'혼돈의 돌':0.4,'더보기 골드':1100}

    if '영원한 빛의 요람 하드3' not in st.session_state:
        st.session_state['영원한 빛의 요람 하드3'] = {'명예의 파편':3000,'정제된 파괴강석':115,'찬란한 명예의 돌파석':4,'혼돈의 돌':0.6,'더보기 골드':1400}

    if '일리아칸 노말1관문' not in st.session_state:
        st.session_state['일리아칸 노말1관문'] = {'명예의 파편':3000,'정제된 파괴강석':120,'찬란한 명예의 돌파석':6,'혼돈의 돌':0.6,'더보기 골드':900}

    if '일리아칸 노말2관문' not in st.session_state:
        st.session_state['일리아칸 노말2관문'] = {'명예의 파편':3000,'정제된 파괴강석':160,'찬란한 명예의 돌파석':8,'혼돈의 돌':0.6,'더보기 골드':1100}
    
    if '일리아칸 노말3관문' not in st.session_state:
        st.session_state['일리아칸 노말3관문'] = {'명예의 파편':4200,'정제된 파괴강석':240,'찬란한 명예의 돌파석':8,'혼돈의 돌':1,'더보기 골드':1500}    

    if '일리아칸 하드1관문' not in st.session_state:
        st.session_state['일리아칸 하드1관문'] = {'명예의 파편':4000,'정제된 파괴강석':200,'찬란한 명예의 돌파석':9,'혼돈의 돌':1.4,'더보기 골드':1200}

    if '일리아칸 하드2관문' not in st.session_state:
        st.session_state['일리아칸 하드2관문'] = {'명예의 파편':4000,'정제된 파괴강석':240,'찬란한 명예의 돌파석':12,'혼돈의 돌':1.4,'더보기 골드':1400}
    
    if '일리아칸 하드3관문' not in st.session_state:
        st.session_state['일리아칸 하드3관문'] = {'명예의 파편':5500,'정제된 파괴강석':360,'찬란한 명예의 돌파석':18,'혼돈의 돌':1.6,'더보기 골드':1900}    

    if '혼돈의 상아탑 노말1관문' not in st.session_state:
        st.session_state['혼돈의 상아탑 노말1관문'] = {'명예의 파편':3000,'정제된 파괴강석':100,'찬란한 명예의 돌파석':2,'선명한 지혜의 기운':2,'더보기 골드':700}

    if '혼돈의 상아탑 노말2관문' not in st.session_state:
        st.session_state['혼돈의 상아탑 노말2관문'] = {'명예의 파편':3000,'정제된 파괴강석':100,'찬란한 명예의 돌파석':2,'선명한 지혜의 기운':2,'더보기 골드':800}

    if '혼돈의 상아탑 노말3관문' not in st.session_state:
        st.session_state['혼돈의 상아탑 노말3관문'] = {'명예의 파편':3000,'정제된 파괴강석':200,'찬란한 명예의 돌파석':2,'선명한 지혜의 기운':3,'더보기 골드':900}

    if '혼돈의 상아탑 노말4관문' not in st.session_state:
        st.session_state['혼돈의 상아탑 노말4관문'] = {'명예의 파편':4500,'정제된 파괴강석':200,'찬란한 명예의 돌파석':3,'선명한 지혜의 기운':1,'선명한 지혜의 엘릭서':1,'더보기 골드':1100}

    if '혼돈의 상아탑 하드1관문' not in st.session_state:
        st.session_state['혼돈의 상아탑 하드1관문'] = {'명예의 파편':4000,'정제된 파괴강석':150,'찬란한 명예의 돌파석':3,'빛나는 지혜의 기운':2,'더보기 골드':1000}

    if '혼돈의 상아탑 하드2관문' not in st.session_state:
        st.session_state['혼돈의 상아탑 하드2관문'] = {'명예의 파편':4000,'정제된 파괴강석':150,'찬란한 명예의 돌파석':3,'빛나는 지혜의 기운':2,'더보기 골드':1000}

    if '혼돈의 상아탑 하드3관문' not in st.session_state:
        st.session_state['혼돈의 상아탑 하드3관문'] = {'명예의 파편':5000,'정제된 파괴강석':240,'찬란한 명예의 돌파석':5,'빛나는 지혜의 기운':3,'더보기 골드':1500}

    if '혼돈의 상아탑 하드4관문' not in st.session_state:
        st.session_state['혼돈의 상아탑 하드4관문'] = {'명예의 파편':5500,'정제된 파괴강석':300,'찬란한 명예의 돌파석':7,'빛나는 지혜의 기운':1,'빛나는 지혜의 엘릭서':1,'더보기 골드':2000}

    break_even = st.session_state[raid]['명예의 파편']*price_sh + st.session_state[raid]['정제된 파괴강석']*price_de +  st.session_state[raid]['찬란한 명예의 돌파석']*price_st - st.session_state[raid]['더보기 골드']
    if break_even >= 0 :
